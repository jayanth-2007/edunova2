...
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET_KEY", "edunova-secret-key")
...
def submit_quiz():
    ...
    weak = [topic for topic, score in mastery_scores.items() if score < 70]
    learning_order = sorted(mastery_scores.keys(), key=lambda t: mastery_scores[t])
    ...
    if client:
        results_text = "\n".join([f"{t}: {s}% mastery" for t, s in mastery_scores.items()])
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You are EduNova's academic advisor AI. "
                            "Analyze quiz results and return ONLY valid JSON with keys: "
                            "weak_topics, learning_order, message."
                        )
                    },
                    {
                        "role": "user",
            ...
            ai_result["weak_topics"] = parsed.get("weak_topics", weak)
            ai_result["learning_order"] = parsed.get("learning_order", learning_order)
            ai_result["message"] = parsed.get("message", ai_result["message"])
        except Exception:
            pass

    users[email]["weak_topics"] = ai_result["weak_topics"]
    users[email]["learning_order"] = ai_result["learning_order"]

    revision = {topic: get_next_review(score) for topic, score in mastery_scores.items()}

    return jsonify({
        "mastery_scores": mastery_scores,
        "weak_topics": ai_result["weak_topics"],
        "learning_order": ai_result["learning_order"],
...
def tutor_chat():
    ...
    if not client:
        ai_reply = f"Demo reply: You asked '{user_message}'. I can help you with {weak_str}."
    else:
        messages = [
            {
                "role": "system",
                "content": (
                    ...
                    f"Teach step by step using simple language and examples. "
...
def dashboard_data():
    ...
    learning_order = user.get("learning_order", [])

    overall = round(sum(mastery.values()) / len(mastery)) if mastery else 0
    revision = {topic: get_next_review(score) for topic, score in mastery.items()}
    ...
    return jsonify({
        ...
        "learning_order": learning_order,
        "revision": revision,
        "topics_mastered": sum(1 for score in mastery.values() if score >= 70),
